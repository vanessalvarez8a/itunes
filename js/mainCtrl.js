...
app.controller('mainCtrl', function($scope, itunesService){
  ...
  $scope.gridOptions = { 
      data: 'songData',
      rowHeight: '100', //ui-grid does not take px or percentages or auto just a plain number
      sortInfo: {fields: ['Song', 'Artist', 'Collection', 'Type'], directions: ['asc']},
      columnDefs: [
        // {field: 'Play', displayName: 'Play', width: '40px', cellTemplate: '<div class="ngCellText" ng-class="col.colIndex()"><a href="{{row.getProperty(col.field)}}"><img src="http://www.icty.org/x/image/Miscellaneous/play_icon30x30.png"></a></div>'},
        {field: 'Artist', displayName: 'Artist'},
        {field: 'Collection', displayName: 'Collection'},
        {field: 'AlbumArt', displayName: 'Album Art', width:'100', cellTemplate: '<div class="ngCellText" ng-class="col.colIndex()"><img ng-src="{{grid.getCellValue(row, col)}}"></div>'},
        {field: 'Type', displayName: 'Type'},
        {field: 'CollectionPrice', displayName: 'Collection Price'},
      ]
  };

  //Our controller is what's going to connect our 'heavy lifting' itunesService with our 
  //view (index.html) so our user can see the results they get back from itunes.

  //First inject itunesService into your controller.

  $scope.getSongData = function() {
    
    itunesService.artist($scope.artist).then(function(songArray){
  ...
  //from the input box on index.html, head over there and check if that input box is tied to 
  //any specific model we could use.
